...
void verify(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B)
{
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
        {
            ...
            {
                std::cout << "Matrx data not match!!!!!!     " << i << ", " << j << ": " << A(i, j) << ", " << B(i, j) << ", " << A(i, j) - B(i, j) << std::endl;
                exit(-1);
            }
            else
            {
                std::cout << "OK  " << i << ", " << j << ": " << A(i, j) << ", " << B(i, j) << ", " << A(i, j) - B(i, j) << std::endl;
            }
        }
    }
}
...
ICP_OUT icp(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B, int max_iterations, float tolerance)
{
    ...
    // 定义存储历史点和残差的队列
    std::deque<Eigen::MatrixXd> x_history; // 历史点
    std::deque<Eigen::MatrixXd> r_history; // 历史残差
    ...
    for (int i = 0; i < max_iterations; i++)
    {
        ...
        Eigen::Vector3f centroid_A(0, 0, 0);
        Eigen::Vector3f centroid_B(0, 0, 0);
        Eigen::MatrixXf A_zm = src3df.transpose();
        Eigen::MatrixXf B_zm = dst_chorder.transpose();

        centroid_A = src3df.rowwise().sum() / src3df.cols();
        centroid_B = dst_chorder.rowwise().sum() / dst_chorder.cols();

        Eigen::MatrixXf H = (A_zm.transpose() * B_zm);
    ...
    // 最大历史深度
    const int history_depth = 5;
    std::deque<Eigen::MatrixXf> x_history; // 历史点
    std::deque<Eigen::MatrixXf> r_history; // 历史残差

    Eigen::MatrixXf r_k = Eigen::MatrixXf::Zero(src.rows(), src.cols()); // 初始化残差

    for (int i = 0; i < max_iterations; i++)
    {
        // 调用 GPU 版本的 ICP（CUDA 加速）
        iter = icp_cuda(dst.transpose(), src3df.transpose(), 1, tolerance, src_next, neighbor);
        ...
        // 确保历史点和残差队列的大小一致
        if (x_history.size() != r_history.size())
        {
            std::cerr << "Error: x_history and r_history sizes are inconsistent!" << std::endl;
            exit(-1);
        }

        // 存储历史点和残差
        x_history.push_back(src);
        r_history.push_back(r_k);

        // 控制历史深度
        if (x_history.size() > history_depth)
        {
            x_history.pop_front();
            r_history.pop_front();
        }
        ...
        if (x_history.size() > 1)
        {
            Eigen::MatrixXf R_matrix(r_history.size(), r_k.rows());
            for (int j = 0; j < r_history.size(); ++j)
            {
                R_matrix.col(j) = r_history[j];
            }

            // 求解权重 alpha
            Eigen::VectorXf alpha = R_matrix.bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(-r_k);

            // 使用历史点加权线性组合更新 src
            Eigen::MatrixXf src_accelerated = Eigen::MatrixXf::Zero(src.rows(), src.cols());
            for (int j = 0; j < x_history.size(); ++j)
            {
                src_accelerated += alpha(j) * x_history[j];
            }

            src = src_accelerated; // 更新 src
            std::cout << "Anderson Acceleration Activated at iteration: " << i << std::endl;
        }

        // 更新 src3df 为当前 src，供下一次迭代使用
        src3df = src(Eigen::seqN(0, 3), Eigen::all);

        // 计算当前误差
        float mean_error = std::accumulate(neighbor.distances.begin(), neighbor.distances.end(), 0.0f) / neighbor.distances.size();
        std::cout << "Mean error at iteration " << i << ": " << mean_error << std::endl;
    ...
    // 打印收敛所花的时间
    std::cout << "Convergence achieved in " << elapsed_time.count() << " seconds." << std::endl;
