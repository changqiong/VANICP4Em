...
ICP_OUT icp(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B, int max_iterations, float tolerance){
    ...
    for (int i=0; i<max_iterations; i++){
        ...
        auto h = chrono::system_clock::now();
	...
	auto h_sub = chrono::system_clock::now();
        neighbor = nearest_neighbor(src3d.transpose(), dst.transpose());
        auto m_sub = chrono::system_clock::now();
	...
	auto m = chrono::system_clock::now();
	chrono::duration<double> diff = m-h;
	chrono::duration<double> diff_sub = m_sub-h_sub;
#if MATRIX_VISIABLE 
	...
	cout<<"Running Time: "<<diff.count()*1000<<" ms."<<endl;
	printf("\n");
	// cout<<"(Non GPU) Iteration Running Time:  "<< diff.count()*1000<<" ms"<<endl;
	// cout<<"(Non GPU) nearest_neighour Running Time:  "<< diff_sub.count()*1000<<" ms"<<endl;
    }
    ...
    double prev_error = 0;
    double mean_error = 0;
    neighbor = nearest_neighbor_cuda(src3df.transpose(), dst.transpose(), true);
    prev_error = std::accumulate(neighbor.distances.begin(),neighbor.distances.end(),0.0)/neighbor.distances.size();
    for (int i=0; i<max_iterations; i++){
        //sleep(1);
        auto h = chrono::system_clock::now();
	...
	auto h_sub = chrono::system_clock::now();
        neighbor = nearest_neighbor_cuda(src3df.transpose(), dst.transpose(), false);
        auto m_sub = chrono::system_clock::now();
        mean_error = std::accumulate(neighbor.distances.begin(),neighbor.distances.end(),0.0)/neighbor.distances.size();
	...
	chrono::duration<double> diff = m-h;
	chrono::duration<double> diff_sub = m_sub-h_sub;
	cout<<"(GPU 0) Iteration Running Time:  "<< diff.count()*1000<<" ms"<<endl;
	cout<<"(GPU 0) nearest_neighour Running Time:  "<< diff_sub.count()*1000<<" ms"<<endl;
    }
